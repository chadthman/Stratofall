...
//		 scores.add(new Score("Broseph", 1337, 4810));
	...
	public void show() {
		...
		outerTable.setBounds(0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
		...
		{
			topHeading = new Label("New High Score!", blackStyleHeaderSmall);
		} else {
		...
		//Add the player score to the inner table
		...
		enterNameWindow.padTop(64).setBounds(0, 0, 500, 500);
		enterNameWindow.add(nameField).minWidth(100).expandX().fillX().colspan(3).row().padBottom(150);
		enterNameWindow.add(windowButtonContinue).expandX().align(Align.center).row();
		//enterNameWindow.pack();
		enterNameWindow.setPosition(Stratofall.WIDTH/2 - enterNameWindow.getWidth()/2, Stratofall.HEIGHT/2 - enterNameWindow.getHeight()/2);
